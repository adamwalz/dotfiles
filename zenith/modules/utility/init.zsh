# Load dependencies.
zenithmodload 'helper' 'spectrum'

# Correct commands.
if zstyle -T ':zenith:module:utility' correct; then
  setopt CORRECT
fi
...
if zstyle -T ':zenith:module:utility' safe-ops; then
...
if [[ ${(@M)${(f)"$(ls --version 2>&1)"}:#*(GNU|lsd) *} ]]; then
  ...
  if zstyle -T ':zenith:module:utility:ls' dirs-first; then
    alias ls="${aliases[ls]:-ls} --group-directories-first"
  fi

  if zstyle -t ':zenith:module:utility:ls' color; then
...
# Grep
if zstyle -t ':zenith:module:utility:grep' color; then
...
# File Download
zstyle -s ':zenith:module:utility:download' helper '_download_helper' || _download_helper='curl'
...
# useful for programs that have their own globbing for
# remote paths.
...
# it's fixed. See https://github.com/sorin-ionescu/zenith/issues/1443 and
# https://github.com/sorin-ionescu/zenith/issues/1521 for more information.
