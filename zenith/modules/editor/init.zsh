...
zstyle -s ':zenith:module:editor' wordchars 'WORDCHARS' \
  || WORDCHARS='*?_-.[]~&;!#$%^(){}<>'
...
function editor-info {
  ...
  # are zenith managed and/or compatible.
  if zstyle -t ':zenith:module:prompt' managed; then
    # Clean up previous $editor_info.
    unset editor_info
    typeset -gA editor_info

    if [[ "$KEYMAP" == 'vicmd' ]]; then
      zstyle -s ':zenith:module:editor:info:keymap:alternate' format 'REPLY'
      editor_info[keymap]="$REPLY"
    else
      zstyle -s ':zenith:module:editor:info:keymap:primary' format 'REPLY'
      editor_info[keymap]="$REPLY"

      if [[ "$ZLE_STATE" == *overwrite* ]]; then
        zstyle -s ':zenith:module:editor:info:keymap:primary:overwrite' format 'REPLY'
        editor_info[overwrite]="$REPLY"
      else
        zstyle -s ':zenith:module:editor:info:keymap:primary:insert' format 'REPLY'
        editor_info[overwrite]="$REPLY"
      fi
    fi
...
function zle-reset-prompt {
  if zstyle -t ':zenith:module:editor' ps-context; then
...
function expand-or-complete-with-indicator {
  local indicator
  zstyle -s ':zenith:module:editor:info:completing' format 'indicator'
...
function _zenith-zle-noop {  ; }
zle -N _zenith-zle-noop
...
for keymap in $unbound_keys; do
  bindkey -M viins "${keymap}" _zenith-zle-noop
  bindkey -M vicmd "${keymap}" _zenith-zle-noop
done
...
for keymap in 'emacs' 'viins'; do
  ...
  # Expand .... to ../..
  if zstyle -t ':zenith:module:editor' dot-expansion; then
...
# Set the key layout.
zstyle -s ':zenith:module:editor' key-bindings 'key_bindings'
if [[ "$key_bindings" == (emacs|) ]]; then
  bindkey -e
elif [[ "$key_bindings" == vi ]]; then
  bindkey -v
else
  print "zenith: editor: invalid key bindings: $key_bindings" >&2
fi
