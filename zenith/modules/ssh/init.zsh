...
_ssh_agent_env="${_ssh_agent_env:-${XDG_CACHE_HOME:-$HOME/.cache}/zenith/ssh-agent.env}"

# Set the path to the persistent authentication socket if not set by another module.
_ssh_agent_sock="${_ssh_agent_sock:-${XDG_CACHE_HOME:-$HOME/.cache}/zenith/ssh-agent.sock}"
...
if ssh-add -l 2>&1 | grep -q 'The agent has no identities'; then
  zstyle -a ':zenith:module:ssh:load' identities '_ssh_identities'
