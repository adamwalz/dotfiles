...
return {
          ...
          utils.on_load('telescope.nvim', function()
            ...
            if not ok then
              ...
              if not vim.uv.fs_stat(lib) then
                warn '`telescope-fzf-native.nvim` not built. Rebuilding...'
                require('lazy').build({ plugins = { plugin }, show = false }):wait(function()
                  info 'Rebuilding `telescope-fzf-native.nvim` done.\nPlease restart Neovim.'
                end)
              else
                error('Failed to load `telescope-fzf-native.nvim`:\n' .. err)
              end
            end
          end)
        end,
  ...
  -- Flash Telescope config
  {
    ...
    opts = function(_, opts)
      if require('lazy.core.config').spec.plugins['flash.nvim'] == nil then
        return
      end
...

  {
    ...
    opts = function()
      ...
      local Keys = require('adam.plugins.lsp_keymaps').get()
      -- stylua: ignore
